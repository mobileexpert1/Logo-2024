...
//        [self insertSubview:self.paopaoView aboveSubview:self.chartScrollView];
...
- (void)drawCircle:(NSInteger)index arr:(NSArray *)pointArr color:(NSArray<UIColor *> *)colors{
    CGFloat pointY = 0;
    for (int i = 0; i<1; i++) {
        NSArray *arr = pointArr.lastObject;
        if (arr.count > index){
            CGPoint point = [arr[index] CGPointValue];
            
            if (point.y>(self.frame.size.height-143)) {
                point.y =18.8;
            }
    ...
    if (self.isFloating) {
        ...
        if (pointY - CGRectGetHeight(frame) - KCircleRadius >=0) {
