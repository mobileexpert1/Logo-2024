...
private enum SimplePDFCommand {
    ...
    case addImage2(UIImage, CGFloat, CGFloat)
...
open class SimplePDF {
    ...
    open func addImage2(_ image: UIImage, width : CGFloat, height : CGFloat) {
        commands += [ .addImage2(image, width, height) ]
    }
    ...
    /// - returns: drawing image rect
    fileprivate func drawImage2(_ image: UIImage, alignment: ContentAlignment, currentOffset: CGPoint, aspectWidth: CGFloat, aspectHeight: CGFloat) -> CGRect {
    ...
    open func generatePDFdata() -> Data {
        ...
        for command in commands {
            ...
            case let .addImage2(image, width, height):
                let imageFrame = drawImage2(image, alignment: alignment, currentOffset: currentOffset, aspectWidth: width, aspectHeight: height)
                lastYOffset = imageFrame.origin.y + imageFrame.height
                switch arrangementDirection {
                case .horizontal:
                    currentOffset = CGPoint(x: imageFrame.origin.x + imageFrame.width, y: currentOffset.y)
